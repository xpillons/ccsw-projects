...
create_config_file() {
    ...
    # level of the config. If nested under libfuse: they are silently ignored, the
    # mount is created without the allow_other kernel option, and non-root users
    # get "permission denied" on the mount point.
    cat > "$config_file" << EOF
